...
class OpenBCIStreamer(CCDLUtil.EEGInterface.EEGInterface.EEGInterfaceParent):
    ...
    def start_recording(self):
        ...
        Breathe.RecordFlag = True
        print 'recordflag is true'

def worker():
    """thread worker function"""
    obs = OpenBCIStreamer(live=True, save_data=True, port='COM16')
    Breathe.RecPrime = time.time()
    obs.start_recording()
    Breathe.RecSecondary = time.time()
    while not Breathe.RecordFlag:
        continue
    print 'hail'
    while time.time() < Breathe.RecSecondary + 3:
        g = obs.out_buffer_queue.get()
        print len(g)
    print 'shows over folx'
    sys.exit()
